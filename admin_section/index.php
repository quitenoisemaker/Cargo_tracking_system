          ...
          <div class="card dashboard text-white bg-success o-hidden">
            ...
            <a class="card-footer text-white clearfix small z-1 text-center" href="#">
              <?php
                  ...
                  $get_total=mysqli_query($conn, "SELECT COUNT(*) FROM `orders`");
          ...
          echo $month ?> Orders</h5></div>
            ...
            <a class="card-footer text-white clearfix small z-1 text-center" href="#">
              <?php
                ...
                $last_day=date('Y-m-t');
                  $get_total=mysqli_query($conn, "SELECT COUNT(*) FROM `orders` WHERE `date_created`>= '$first_day' AND `date_created`<= '$last_day'");
                $row_total=mysqli_fetch_array($get_total);
        ...
        <div class="col-xl-3 col-sm-6 mb-3">
          <div class="card dashboard text-white bg-primary o-hidden">
            ...
            <a class="card-footer text-white clearfix small z-1 text-center" href="#">
                ...
                <?php
                  $total=0;
                  $get_total=mysqli_query($conn, "SELECT * FROM `orders`");
                while ( $row_total=mysqli_fetch_array($get_total)) {
                  $total+=$row_total['price'];
                }
                
                echo "<p class='' style='font-size: 27px'>NGN " .number_format($total)."</p>"; 
