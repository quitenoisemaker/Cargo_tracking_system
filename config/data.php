		...
		if(!empty($_POST['pay_email'])){
            ...
            $get_email22=mysqli_query($conn, "SELECT * FROM contact");
                ...
                $message.='<div style="text-align: center;">
                        <div class="" >
                                            ...
                                            <h4>Name: '.$row_customer22['customer_fname'].' '.$row_customer22['customer_lname'].'</h4>
                                            <h4> Phone: '.$row_customer22['customer_contact'].'</h4>
                                            <h4> Email: '.$row_customer22['customer_email'].'</h4>
                                            <h4> Address: '.$row_customer22['customer_address'].'</h4>
                                        ...
                                        <tbody>';
                                            ...
                                            $count=0;
                                          $get_terminal = mysqli_query($conn, "SELECT * FROM `cart` crt JOIN `products` pr WHERE crt.ip_add='$ip' AND crt.p_id= pr.product_id");

                                
                                        while ($row = mysqli_fetch_array($get_terminal)) {

                                      $message.='<tr>
                                                <th><img src="'.$row_seo22['site_link'].'/admin_section/product_images/resized_'.$row['product_image'].' " width="35"></th>
                                                <th>'.$row['product_title'].'</th>
                                                <th><big style="color: red"> '.number_format($row['product_price']).'   </big></th>
                                                <th>'.$row['qty'] .'</th>
                                                <th>'.$row['size'] .'</th>
                                                <th>'.$row['color'] .'</th>
                                            </tr>';
                                            
                                            $count+=$row['product_price'] * $row['qty'];
                                            } 
            ...
            $sql = "SELECT * FROM cart where ip_add='$ip'";
                 ...
                 while($row_product=$result2->fetch_assoc()){ 
                 	...
                 	mysqli_query($conn,"INSERT INTO `orders`(`id`, `product_id`,`fname`,`lname`, `customer_email`,`ip`, `items`,`img`, `qty`, `price`,`color`,`size`,`payment_method`, `date_created`) VALUES (NULL,'$row_product[product_id]','$row_customer22[customer_fname]','$row_customer22[customer_lname]','$_SESSION[email]','$ip','$row_product[product_title]','$row_product[product_image]','$row[qty]','$row_product[product_price]','$row[color]','$row[size]','online payment','$date')");
...
if(isset($_GET['action']) && $_GET['action']=='delivery_wallet'){
		...
		if(!empty($_POST['pay_email'])){
            ...
            $get_email22=mysqli_query($conn, "SELECT * FROM contact");
                ...
                $message.='<div style="text-align: center;">
                        <div class="" >
                                            <h4 style="color: red">PAY ON DELIVERY ORDER SUMMARY</h4>
                                            <style>
                                            table, th, td {
                                              border: 1px solid black;
                                              padding: 5px;
            ...
            $sql = "SELECT * FROM cart where ip_add='$ip'";
                 ...
                 while($row_product=$result2->fetch_assoc()){ 
                 	...
                 	mysqli_query($conn,"INSERT INTO `orders`(`id`, `product_id`,`fname`,`lname`, `customer_email`,`ip`, `items`,`img`, `qty`, `price`,`color`,`size`,`payment_method`, `date_created`) VALUES (NULL,'$row_product[product_id]','$row_customer22[customer_fname]','$row_customer22[customer_lname]','$_SESSION[email]','$ip','$row_product[product_title]','$row_product[product_image]','$row[qty]','$row_product[product_price]','$row[color]','$row[size]','Pay on Delivery','$date')");
